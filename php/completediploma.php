...
if (($_SERVER["REQUEST_METHOD"] == "POST")  && (isset($_POST["am"])) && (isset($_POST["title"]))) {
  ...
  if ($result = $conn->query($query)) {
    ...
    if ($result = $conn->query($query)) {
      $row = $result->fetch_array(MYSQLI_ASSOC);
      $teacherid = $row["id"];
      $query = "UPDATE create_project SET DateCompleted = CURDATE() WHERE project_id = '$projectid' AND teacher_id = '$teacherid'";
      if ($conn->query($query)) {
        $query = "UPDATE interest_project SET Status = 5 WHERE project_id = '$projectid'";
        if ($conn->query($query)) {
          echo "true";
        }
        else {
