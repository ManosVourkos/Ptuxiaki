...
function previewTags(data) {
  if(data.length == 0) {
    ...
    $.each( data, function( key, val ) {
        ...
        text += val.Name;
        text += "</span><div><span class=\"settings pr-1\">Δημιουργήθηκε από</span><span class=\"settings\">";
        text += val.Professor;
        text += "</span></div><div class=\"\"><span class=\"settings pr-1\">";
        if (val.Count == 0) {
     ...
     success: function(data)
     {
       if (data != false) {
         if(data.hasOwnProperty("error")){
           ...
           $("#main-content").html("<hr/><div id=\"loading\" class=\"loading\"><div class=\"spinner-border\" role=\"status\"><span class=\"sr-only\">Loading...</span></div><span>Φορτωση...</span></div>");
